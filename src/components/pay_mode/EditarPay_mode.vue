<template>
    <div class="container text-start">
        ...
        <div class="card">
           ...
           <div class="card-body">
            <form @submit.prevent="updatePay_mode">
             <div class="row mb-3">
...
export default{
    ...
    methods:{
        ...
        async updatePay_mode(){
            const res =await axios.put(`http://127.0.0.1:8000/api/pay_mode/${this.pay_mode.id}`,this.pay_mode)
            if(res.status==200){
                this.$router.push({name:'Pay_mode'})
                Swal.fire({
                    icon: 'success',
                    title: 'Pay_mode editado ',
    ...
    mounted(){
        ...
        axios.get(`http://127.0.0.1:8000/api/pay_mode/${this.pay_mode.id}`)
